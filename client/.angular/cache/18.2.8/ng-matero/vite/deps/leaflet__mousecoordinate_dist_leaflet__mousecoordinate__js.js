// node_modules/leaflet.mousecoordinate/dist/leaflet.mousecoordinate.js
L.Control.mouseCoordinate = L.Control.extend({
  ...
  onAdd: function(map) {
    this._map = map;
    if (L.Browser.touch || L.Browser.msTouch) return L.DomUtil.create("div");
  ...
  _update: function(e) {
    var lat = Math.round(e.latlng.lat * 1e5) / 1e5;
    var lng = Math.round(e.latlng.lng * 1e5) / 1e5;
    var gps = {
      lat,
      lng
    };
  ...
  _geo2geodeziminuten: function(gps) {
    ...
    var latminuten = Math.round((gps.lat - latgrad) * 60 * 1e4) / 1e4;
    var lnggrad = parseInt(gps.lng);
    var lngminuten = Math.round((gps.lng - lnggrad) * 60 * 1e4) / 1e4;
...
var UTM = {
  ...
  fromLatLng: function(latlng) {
    ...
    var nw;
    if (bw < 0) {
      nw = 1e7 + 0.9996 * (g + nd * cos2 * tan1 * dl2 / 2 + nd * cos4 * tan1 * (5 - tan2 + 9 * etasq) * dl4 / 24);
    } else {
      nw = 0.9996 * (g + nd * cos2 * tan1 * dl2 / 2 + nd * cos4 * tan1 * (5 - tan2 + 9 * etasq) * dl4 / 24);
    }
    var ew = 0.9996 * (nd * cos1 * dl + nd * cos3 * (1 - tan2 + etasq) * dl3 / 6 + nd * cos5 * (5 - 18 * tan2 + tan4) * dl5 / 120) + 5e5;
    var zone = lz + bz;
...
//# sourceMappingURL=leaflet__mousecoordinate_dist_leaflet__mousecoordinate__js.js.map
